...
// Card de conexão do Instagram (via Zernio). Vive dentro de Canais: era a única
// tela que gravava instagram_access_token e ficou inalcançável quando
// Credenciais saiu do router. O token entra cifrado no banco (CRYPTO_KEY) via
// /api/credentials — nunca em .env, nunca no front depois de salvo.
export function InstagramCard({ onSaved }: { onSaved?: () => void }) {
  ...
  return (
    <div className="rounded-xl border border-[rgba(var(--accent-rgb),0.15)] bg-[rgba(var(--surface-rgb),0.02)] p-5 backdrop-blur-[40px]">
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-medium uppercase tracking-[0.1em] text-[var(--text-secondary)]">
              Instagram
            </span>
            {connected === null ? (
          ...
          <div className="mt-4 flex flex-col gap-2 sm:flex-row">
            <input
              ...
              className="flex-1 rounded-lg border border-[rgba(var(--accent-rgb),0.2)] bg-[rgba(var(--surface-rgb),0.03)] px-3 py-2 text-sm text-[var(--color-text-primary)] outline-none focus:border-[var(--accent-primary)]"
            />
            <button
              onClick={save}
