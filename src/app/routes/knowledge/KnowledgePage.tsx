        ...
        <div className="md:hidden space-y-2">
          {loading ? (
            ...
            items.map((it) => {
              ...
              return (
                <div key={it.id} className="rounded-lg border border-[rgba(var(--accent-rgb),0.1)] bg-[rgba(var(--surface-rgb),0.02)] p-3">
                  <div className="flex items-start gap-2">
                    <Icon className="h-4 w-4 shrink-0 text-[var(--color-text-secondary)] mt-0.5" />
                    <div className="min-w-0 flex-1">
                      <div className="font-semibold text-[var(--color-text-primary)] truncate">{it.name}</div>
                      {it.source_url && (
                        <div className="text-[11px] font-mono text-[var(--color-text-secondary)] truncate">{it.source_url}</div>
                      )}
                    </div>
                    <Button size="icon" variant="ghost" onClick={() => handleDelete(it)} aria-label="Remover">
            ...
            <tbody>
              {loading ? (
                ...
                items.map((it) => {
                  const Icon = it.type === 'url' ? Globe : FileText;
                  return (
                    <tr key={it.id} className="border-t border-[rgba(var(--accent-rgb),0.06)]">
                      <td className="p-3">
      ...
      <form onSubmit={handle} className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {([
              ...
              <button
                ...
                className={
                  sourceType === id
                    ? 'p-3 rounded-lg border border-[var(--accent-primary)] bg-[rgba(var(--accent-rgb),0.08)] text-sm'
                    : 'p-3 rounded-lg border border-[rgba(var(--accent-rgb),0.12)] bg-[rgba(var(--surface-rgb),0.02)] text-sm text-[var(--color-text-secondary)]'
                }
                disabled={submitting}
              >
        ...
        {sourceType === 'text' && (
          <div className="space-y-2">
            ...
            <textarea
              ...
              className="w-full rounded-lg border border-[rgba(var(--accent-rgb),0.2)] bg-[rgba(var(--surface-rgb),0.03)] px-4 py-3 text-sm text-[var(--color-text-primary)] focus:outline-none focus:border-[var(--accent-primary)]"
            />
          </div>
        )}
        ...
        {sourceType === 'pdf' && (
          <div className="space-y-2">
            ...
            <label
              htmlFor="kb_file"
              className="flex items-center gap-3 h-11 px-4 rounded-lg border border-dashed border-[rgba(var(--accent-rgb),0.2)] bg-[rgba(var(--surface-rgb),0.03)] cursor-pointer hover:border-[rgba(var(--accent-rgb),0.4)]"
            >
