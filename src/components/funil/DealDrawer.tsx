...
export function DealDrawer({ deal, stages, pipelines, isAdmin, onClose, onStageChange, onChanged }: DealDrawerProps) {
  ...
  const markWon = () => {
    ...
    if (defaultPipeline && deal.pipeline_id === defaultPipeline.id) {
      const wonStage = stages.find((s) => s.is_won);
      if (wonStage && wonStage.id !== deal.stage_id) {
        patch.stage_id = wonStage.id;
      ...
      >
        <div className="sticky top-0 z-10 flex items-start justify-between gap-3 border-b border-[rgba(var(--accent-rgb),0.12)] bg-[var(--bg-primary)]/95 px-5 py-4 backdrop-blur">
          <div className="min-w-0 flex-1">
            ...
            />
            {/* Subtítulo = produtos comprados (multi-select do catálogo) */}
            <ProductMultiSelect
              selected={products}
              catalog={productCatalog}
              onAdd={async (name) => { await detail.addProduct(name); void onChanged(); }}
              onRemove={async (id) => { await detail.removeProduct(id); void onChanged(); }}
            />
          </div>
          <button onClick={onClose} aria-label="Fechar" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg text-[var(--color-text-secondary)] transition hover:bg-[rgba(var(--surface-rgb),0.05)] hover:text-[var(--color-text-primary)]">
            <X className="h-5 w-5" />
          </button>
        </div>
          ...
          <div className="space-y-6 p-5">
            ...
            <section className="space-y-2">
              ...
              {deal.status === 'open' ? (
                <div className="space-y-2">
                  ...
                  {lostReasonOpen && (
                    <div className="space-y-2 rounded-lg border border-[rgba(239,68,68,0.2)] bg-[rgba(239,68,68,0.05)] p-3">
                      ...
                      <button
                        onClick={markLost}
                        className="w-full rounded-lg bg-[#EF4444] py-2 text-sm font-semibold text-[var(--on-accent)] transition hover:opacity-90"
                      >
                        Confirmar perda
                      </button>
                    </div>
                ...
                <div className="space-y-2 rounded-lg border border-[rgba(var(--accent-rgb),0.12)] bg-[rgba(var(--surface-rgb),0.02)] p-3">
              ...
              {origin ? (
                <div className="space-y-3 rounded-lg border border-[rgba(var(--accent-rgb),0.12)] bg-[rgba(var(--surface-rgb),0.02)] p-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {origin.channel && (
                      <span className="text-sm font-semibold text-[var(--color-text-primary)]">{origin.channel}</span>
                    )}
                  </div>
                  <dl className="grid grid-cols-2 gap-x-3 gap-y-2">
                    <OriginRow label="Campanha" value={deal.utm_campaign} />
                    <OriginRow label="Anúncio / Conteúdo" value={deal.utm_content} />
                    <OriginRow label="utm_source" value={deal.utm_source} />
                    <OriginRow label="utm_medium" value={deal.utm_medium} />
                    <OriginRow label="Termo (utm_term)" value={deal.utm_term} />
                  </dl>
                </div>
              ) : (
            ...
            <Link to={`/inbox?contact=${deal.contact_id}`} className="flex items-center justify-center gap-2 rounded-lg border border-[rgba(var(--accent-rgb),0.2)] py-2.5 text-sm font-medium text-[var(--accent-secondary)] transition hover:border-[var(--accent-primary)] hover:bg-[rgba(var(--surface-rgb),0.05)]">
...
// Subtítulo do drawer: produtos que o lead comprou, como dropdown de múltipla
// escolha sobre o catálogo (Configurações → Produtos). Aberto, o drawer exibe
// todos os selecionados; o card minimizado do funil resume ("X e outros").
function ProductMultiSelect({
  ...
  const label = selected.length > 0 ? selected.map((p) => p.name).join(', ') : 'Selecionar produtos…';
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <ChevronDown className={`mt-0.5 h-3.5 w-3.5 shrink-0 transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && (
        <>
          <div className="fixed inset-0 z-10" onClick={() => setOpen(false)} />
          <div className="absolute z-20 mt-1 max-h-60 w-full overflow-y-auto rounded-lg border border-[rgba(var(--accent-rgb),0.25)] bg-[var(--bg-primary)] p-1 shadow-[0_0_30px_rgba(var(--accent-rgb),0.15)]">
            {catalog.length === 0 ? (
              ...
              catalog.map((p) => {
                ...
                return (
                  <button
                    ...
                    className="flex w-full items-center gap-2 rounded-md px-3 py-1.5 text-left text-sm text-[var(--color-text-primary)] hover:bg-[rgba(var(--surface-rgb),0.05)]"
                  >
                    <span
                    ...
                    >
                      {active && <Check className="h-3 w-3 text-[var(--on-accent)]" />}
                    </span>
  ...
  return (
    <div>
      <input
        ...
        className="w-full rounded border border-[rgba(var(--accent-rgb),0.3)] bg-[rgba(var(--surface-rgb),0.05)] px-2 py-1 text-sm text-[var(--color-text-primary)] outline-none focus:border-[var(--accent-primary)]"
      />
...
function ChipEditor({
  ...
  label: string;
  chips: { id: string; name: string; color?: string; meta?: string }[];
    ...
    <section className="space-y-2">
        ...
        <div className="flex flex-wrap gap-1.5">
          {chips.map((c) => (
            ...
            >
              ...
              {c.meta && <span className="font-semibold text-[var(--accent-secondary)]">{c.meta}</span>}
      ...
      <div className="relative">
        ...
        {open && (q.trim() || suggestions.length > 0) && (
          <>
            <div className="fixed inset-0 z-10" onClick={() => setOpen(false)} />
            <div className="absolute z-20 mt-1 w-full rounded-lg border border-[rgba(var(--accent-rgb),0.25)] bg-[var(--bg-primary)] p-1 shadow-[0_0_30px_rgba(var(--accent-rgb),0.15)]">
              {suggestions.map((s) => (
                <button key={s.id} onClick={() => void add(s.name)} className="flex w-full items-center gap-2 rounded-md px-3 py-1.5 text-left text-sm text-[var(--color-text-primary)] hover:bg-[rgba(var(--surface-rgb),0.05)]">
                  {s.name}
                </button>
              ))}
              {q.trim() && !exact && (
                <button onClick={() => void add(q.trim())} className="flex w-full items-center gap-2 rounded-md px-3 py-1.5 text-left text-sm text-[var(--accent-secondary)] hover:bg-[rgba(var(--surface-rgb),0.05)]">
    ...
    <div>
      ...
      {field.field_type === 'boolean' ? (
        <label className="inline-flex cursor-pointer items-center gap-2 text-sm text-[var(--color-text-primary)]">
          <input type="checkbox" checked={local === 'true'} onChange={(e) => { const v = e.target.checked ? 'true' : 'false'; setLocal(v); commit(v); }} className="h-4 w-4 accent-[var(--accent-primary)]" />
          {local === 'true' ? 'Sim' : 'Não'}
        </label>
      ...
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-sm rounded-2xl border border-[rgba(var(--accent-rgb),0.25)] bg-[var(--bg-primary)] p-5 shadow-[0_0_40px_rgba(var(--accent-rgb),0.15)]">
        ...
        <div className="space-y-3">
          ...
          <label className="flex cursor-pointer items-center gap-2 text-sm text-[var(--color-text-primary)]">
            <input type="checkbox" checked={required} onChange={(e) => setRequired(e.target.checked)} className="h-4 w-4 accent-[var(--accent-primary)]" /> Obrigatório
          </label>
          {err && <div className="text-xs text-[#EF4444]">{err}</div>}
        </div>
        <div className="mt-5 flex gap-2">
          <button onClick={submit} disabled={busy} className="flex-1 inline-flex items-center justify-center gap-1 rounded-lg bg-gradient-to-br from-[var(--accent-deep)] to-[var(--accent-primary)] px-3 py-2 text-sm font-semibold text-[var(--on-accent)] transition hover:opacity-90 disabled:opacity-60">
