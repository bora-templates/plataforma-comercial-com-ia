...
export function TemplateFormDialog({
  ...
  // Exemplos por variável ({{1}} → texto de amostra exigido pela Meta na aprovação).
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Variáveis são permitidas ({{1}}, {{2}}…) — a Meta exige um exemplo por
    // variável para aprovar o template.
    if (detectedVars.some((i) => !(varExamples[String(i)] ?? '').trim())) {
      toast.error('Preencha o exemplo de cada variável', {
        description: 'A Meta exige um texto de amostra por variável para aprovação.',
      });
      return;
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="rounded-lg border border-[rgba(139,92,246,0.25)] bg-[rgba(139,92,246,0.05)] p-4 space-y-3">
          ...
          <textarea
            ...
            rows={2}
            className="w-full rounded-lg border border-[rgba(139,92,246,0.2)] bg-[rgba(var(--surface-rgb),0.03)] px-4 py-2 text-sm text-[var(--color-text-primary)] focus:outline-none focus:border-[#A78BFA]"
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            rows={5}
            disabled={saving}
            className="w-full rounded-lg border border-[rgba(var(--accent-rgb),0.2)] bg-[rgba(var(--surface-rgb),0.03)] px-4 py-3 text-sm font-mono text-[var(--color-text-primary)] focus:outline-none focus:border-[var(--accent-primary)]"
          />
          <div className="flex items-center justify-between gap-2">
            <p className="text-[11px] text-[var(--color-text-secondary)] opacity-70">
              Use {'{{1}}'}, {'{{2}}'}… para variáveis. Máximo ~1024 caracteres (limite Meta).
            </p>
            <button
              type="button"
        ...
        {detectedVars.length > 0 && (
          <div className="space-y-2 rounded-lg border border-[rgba(var(--accent-rgb),0.2)] bg-[rgba(var(--surface-rgb),0.02)] p-3">
            ...
            {detectedVars.map((i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <Input
                  ...
                  placeholder={`Exemplo p/ variável ${i} (ex.: João)`}
                  disabled={saving}
                />
              </div>
            ))}
